...
```
Rogue/
├── assets/fontawesome/     # Font Awesome 6 图标库（本地离线）
│   ├── all.min.css
│   └── webfonts/           # 8个字体文件
├── assets/bgm/             # BGM 音频（当前禁用，文件保留）
├── abyss-labyrinth.html    # 游戏主文件（~7000行，HTML+CSS+JS）
├── README.md               # 项目说明
├── CHANGELOG.md            # 版本日志
...
**本游戏为单机游戏，所有资源必须本地化，不允许任何网络请求。**

- ❌ 禁止 CDN 外链（CSS/JS/字体/图标/图片等）
- ❌ 禁止 fetch / XMLHttpRequest 等网络请求
- ❌ 禁止 `<link>` 或 `<script>` 引用外部 URL
- ✅ 所有第三方库/资源下载到 `assets/` 目录，使用相对路径引用
- ✅ 图标库已本地化：`assets/fontawesome/all.min.css`（Font Awesome 6.7.2）

**新增第三方资源的步骤：**
1. 下载到 `assets/` 对应子目录
2. 用相对路径引用（如 `assets/xxx/yyy.css`）
3. 更新本文件的项目结构图和 CHANGELOG
...
### 15. 城堡装饰物碰撞
装饰物（火炬🔥/石柱🏛️/旗帜🚩/王座👑/吊灯🕯️ → 已升级为 FA 图标）存储在 `gameState.castleDecorations` 数组中。`movePlayer()` 城堡段在 NPC 检查之前遍历装饰物数组做碰撞检测。注意此检查必须在墙壁检查之后、NPC交互之前，因为装饰物虽不可穿越但不影响 NPC 交互的优先级。红毯 ◆ 走 `castleCarpet` 数组，仅影响渲染不阻挡移动。
